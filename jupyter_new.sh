...
# Everyone needs pandas
pip install pandas

if [[ "${ROLE}" == 'Master' ]]; then
  ./dataproc-initialization-actions/jupyter/internal/setup-jupyter-kernel.sh
  # This snippet is brought to you by the bad idea bears.
  sudo mkdir -p /root/.jupyter || echo "already have config"
  sudo gsutil cp gs://asf-diversity-data/jupyter_config_hacks.py /root/.jupyter/
  sudo chmod 777 /root/.jupyter/jupyter_notebook_config.py
  cat /root/.jupyter/jupyter_config_hacks.py >> /root/.jupyter/jupyter_notebook_config.py
  # End of bad idea bears snippet
